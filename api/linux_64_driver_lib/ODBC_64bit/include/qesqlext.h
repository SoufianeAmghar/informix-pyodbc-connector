...
**
** (C) Copyright 1993-2021 By Progress Software Corporation.
** All Rights Reserved.
*/
...
#define SQL_COPT_REPLICATION_USER					(SQL_CONOPT_START+40) /* Used to decide if Replication User is enabled (Specific to SQLServer)*/


#define SQL_CONOPT_END						(SQL_CONOPT_START+99) 
...
/* inserts data with redo or undo logging, default TRUE */
#define SQL_ATTR_BULK_ORA_REDOLOGGING			(SQL_STMTOPT_START+18)

/* a partition or subpartition load being done as opposed to an entire table load, default FALSE (whole table lock) */
#define SQL_ATTR_BULK_ORA_PARALLEL				(SQL_STMTOPT_START+19)

/* a table's partition or sub-partition name, default NULL or empty */
#define SQL_ATTR_BULK_ORA_SUBNAME				(SQL_STMTOPT_START+20)

//Individual drivers may not support these attributes below unless they are modified to use them at the stmt level.

/* Error toerance count for bulk load operation of SQLINTEGER value >= -1 */
/* It overwrites the connection level BulkLoadErrorTolerance (BLET) connection option, default -1 (unlimited) */
#define SQL_ATTR_BULK_ERROR_TOLERANCE			(SQL_STMTOPT_START+21)

//Ability to enable / disable bulk load through an ODBC statement attribute
#define SQL_ATTR_BULK_LOAD_ENABLED				(SQL_STMTOPT_START+22)

//Ability to set BulkLoadWriteBufferSize through an ODBC statement attribute
#define SQL_ATTR_BULK_LOAD_WRITE_BUFFER_SIZE	(SQL_STMTOPT_START+23)

//Ability to set IANAAppCodePage through an ODBC statement attibute
#define SQL_ATTR_IANA_APP_CODE_PAGE				(SQL_STMTOPT_START+24)

/* SQLServer statement attribute for enabling/disabling column encryption */
#define SQL_ATTR_SS_COLUMN_ENCRYPTION			(SQL_STMTOPT_START+25)
/* Defines for use with SQL_ATTR_SS_COLUMN_ENCRYPTION */
#define SQL_CE_DISABLED						0L			// Always Encrypted is disabled for the statement 
#define SQL_CE_ENABLED						1L			// Always Encrypted is enabled and used for both parameters and results		
#define SQL_CE_RESULTSETONLY				2L			// Decryption Only. Resultsets and return values are decrypted, and parameters are not encrypted 

#define SQL_STMTOPT_END							(SQL_STMTOPT_START+25)
